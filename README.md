...
## النشر على استضافة سحابية (Render)

لتشغيل المشروع 24/7 على استضافة حقيقية بدل جهازك الشخصي، راجع الدليل المخصص: [`DEPLOY_RENDER.md`](DEPLOY_RENDER.md) — يستخدم ملف `render.yaml` الجاهز بجذر المستودع لنشر كل الخدمات الست (API، البوت، الـ Worker، لوحة الإدارة، PostgreSQL، Redis) تلقائياً.
